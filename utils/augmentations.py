...
def intersect(box_a, box_b):
    """
    :param box_a: Shape: [num_boxes, 4]
    :param box_b: Shape: [4] (xmin,ymin,xmax,ymax)
    :return: intersect. Shape: [num_boxes]
    """
...
class ToAbsoluteCoords(object):
    """
    Convert box coordinate to absolute
    """
...
class ToPercentCoords(object):
    """
    Convert box coordinate to percent (% width, height of image)
    """
...
class RandomSampleCrop(object):
    ...
    Return:
        (img, boxes, classes)
            img (Image): the cropped image from random sampled patch from right,bottom conner original image
    ...
    def __init__(self):
        self.sample_options = (
            # using entire original input image, dont crop
            None,
        ...
        while True:
            ...
            for _ in range(50):
                ...
                current_image = current_image[
                                rect[1]:rect[3],  # ymin -> ymax
                                rect[0]:rect[2],  # ximn -> xmax
                                :]

                # Giữ các ground truth box mà center nằm trong sampled patch
                ...
                current_boxes = boxes[mask, :].copy()  # xmin-xmax
    ...
    def __init__(self, size=300, mean=(104, 117, 123)):
        ...
        if das:
            self.augment = Compose([
                ...
                PhotometricDistort(),
                # RandomSampleCrop(),
                RandomBaiduCrop(self.size),  # different point
                # Expand(self.mean),
                RandomMirror(),
                ToPercentCoords(),
                Resize(self.size),
            ...
            self.augment = Compose([
                ...
                PhotometricDistort(),
                # Expand(self.mean),
                RandomCrop(self.size),      # different point
                RandomMirror(),
                ToPercentCoords(),
                Resize(self.size),
                SubtractMeans(self.mean)
